const MIC_TYPES = {
    'Omnidirectional': {
        name: "全向性",
        formula: (k, theta) => k * 1,
        // 舊: "收音均勻分佈於 360°，適合環境錄音，但無法有效抑制噪音。"
        desc: "廣角收音：四面八方的聲音都收，雖然很自然，但降噪效果差。"
    },
    'Cardioid': {
        name: "心型",
        formula: (k, theta) => k * (0.5 + 0.5 * Math.cos(theta)),
        // 舊: "主要收音在前方。後方 180° 衰減最強，常見於手持麥克風。"
        desc: "前方收音：專注於前方，對後方的噪音有很強的隔絕效果。"
    },
    'Supercardioid': {
        name: "超心型",
        formula: (k, theta) => k * (0.37 + 0.63 * Math.cos(theta)),
        // 舊: "指向性更強，適合舞台。後方會出現小收音峰，但側面衰減更好。"
        desc: "超級專注：比心型更集中於前方，但後面會稍微收一點點聲音。"
    },
    'Shotgun': {
        name: "槍型",
        formula: (k, theta) => k * (0.1 + 0.9 * Math.cos(theta)),
        // 舊: "極超指向性。收音範圍極窄，用於遠距離收音或極端降噪，有長干涉管。"
        desc: "遠距離收音：收音範圍極窄，適合隔遠距離或在極吵雜環境下使用。"
    }
};
